...
public class MinersPickaxeRecipe extends Recipes {
    private static ItemStack diamond = new ItemStack(Material.DIAMOND_BLOCK);

    private static ItemStack beacon = new ItemStack(Material.BEACON);

    private static ItemStack emerald = new ItemStack(Material.EMERALD_BLOCK);

    private static ItemStack stick = new ItemStack(Material.BLAZE_ROD);
    ...
    private static ItemStack result = MinersPickaxe.item();

    public static void show(Player player) {
        Inventory inventory = Recipes.inventory;
        Recipes.show(player);
        inventory.setItem(1, diamond);
        inventory.setItem(2, beacon);
